...
| ~~Creating and editing leads through the UI~~ — **create was reversed by spec 0003** | Still true of *editing*. The create half was reversed: see `docs/specs/0003-add-lead-form.md`, which adds a single-lead form. The original reasoning held — it is a second feature with its own validation rules — which is why it got its own spec rather than being bolted on here. | Create: built in spec 0003. Edit: still only the row actions. |
| Deleting leads | Explicitly ruled out — "nothing gets deleted". Archive is the delete. | Status `Archived`. |
| ~~An archive/unarchive row action~~ — **reversed by spec 0003** | Not among the three actions named here. Reversed once a create form made mistakes easy to produce with no undo, and because `status` was a column the app displayed with no way to change. | Built in spec 0003. |
